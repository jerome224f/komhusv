...
import { Building2, Users, CalendarCheck, CalendarOff, FileSpreadsheet, AlertCircle, Loader2 } from 'lucide-react';
...
export function Dashboard() {
  ...
  const [absentEmployeesToday, setAbsentEmployeesToday] = useState<{employee: Employee, org: Organization}[]>([]);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      {absentEmployeesToday.length > 0 && (
        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl shadow-sm flex items-start gap-4">
          ...
          <div>
            <h3 className="text-red-800 font-bold">Attention: {absentEmployeesToday.length} Absent Employee{absentEmployeesToday.length > 1 ? 's' : ''} Today</h3>
            <p className="text-red-600 text-sm mt-1">Please review the list of absentees and take necessary actions.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {statCards.map((stat, i) => {
          ...
          return (
            <div key={i} className={`bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden ${hasAbsentees ? 'ring-2 ring-red-500 ring-offset-2' : ''}`}>
              {hasAbsentees && (
              ...
              <ul className="divide-y divide-gray-100">
                {absentEmployeesToday.map((item, idx) => (
                  <li key={idx} className="p-4 flex justify-between items-center bg-white hover:bg-gray-50 transition-colors">
                    ...
                    <span className="px-3 py-1 bg-red-50 text-red-700 text-xs font-semibold rounded-full border border-red-100">Absent</span>
                  </li>
                ))}
              </ul>
            )}
        ...
        <div className="pb-2">
          <div className="w-full">
            ...
            <div className="grid grid-cols-7 gap-2">
              {heatmapData.length > 0 && Array.from({ length: new Date(heatmapData[0].date + 'T00:00:00').getDay() }).map((_, i) => (
